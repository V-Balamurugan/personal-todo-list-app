...
import { AlertCircle, RotateCcw, ArrowRight, Calendar } from 'lucide-react';
import { useTodos } from '../../context/TodoContext';

export const UrgentAlertsCard: React.FC = () => {
  const { overdueTodos, overdueCount, setActiveView, rescheduleToToday, rescheduleToTomorrow } = useTodos();
  ...
  const handleRescheduleTomorrow = async () => {
    ...
    setIsRescheduling(true);
    try {
      await rescheduleToTomorrow(firstOverdue.id);
    } finally {
      setIsRescheduling(false);
    }
  };
    ...
    <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-rose-500/10 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-slide-down">
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
        {firstOverdue && (
          <>
            <button
              ...
              className="flex-1 sm:flex-initial py-2 px-3 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition-all"
            >
            ...
            <button
              ...
              disabled={isRescheduling}
              className="flex-1 sm:flex-initial py-2 px-3 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition-all"
            >
        ...
        <button
          ...
          className="flex-1 sm:flex-initial py-2 px-3 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-semibold rounded-xl flex items-center justify-center gap-1 transition-all"
        >
          <span>View All</span>
