...
import { getSampleTodos, type DbStatusDetail } from '../../services/todoService';
...
export const SettingsView: React.FC = () => {
  ...
  const [dbStatus, setDbStatus] = useState<DbStatusDetail>({
    status: 'local_fallback',
    message: 'Initializing...',
  });

  const firebaseConfig = getFirebaseConfig();
  const projectId = firebaseConfig?.projectId || 'todo-app-d8285';

  useEffect(() => {
    const handleStatus = (e: Event) => {
      const customEvent = e as CustomEvent<DbStatusDetail>;
      if (customEvent.detail) {
        setDbStatus(customEvent.detail);
      }
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12 animate-fade-in">
      ...
      {/* 3. Firebase & Database Synchronization */}
      <div className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <span
            className={`px-2.5 py-1 rounded-full text-xs font-bold border flex items-center gap-1.5 ${
              dbStatus.status === 'connected'
                ...
                : 'bg-amber-50 text-amber-800 dark:bg-amber-950/80 dark:text-amber-300 border-amber-200 dark:border-amber-900'
            }`}
          >
            {dbStatus.status === 'connected' ? (
              <>
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                <span>Realtime Database Live</span>
              </>
            ) : (
              <>
        ...
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
          {dbStatus.status === 'connected'
            ? `Your tasks are securely synchronized in real-time to your Firebase Realtime Database (Project: ${projectId}).`
            : `All tasks are safely stored in your browser's persistent database. Even when offline, your todo items, subtasks, and progress are 100% saved.`}
        </p>

        {dbStatus.status === 'local_fallback' && (
          <div className="p-4 rounded-2xl bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 text-xs space-y-2 text-slate-700 dark:text-slate-300">
            <p className="font-semibold text-amber-900 dark:text-amber-200 flex items-center gap-1.5">
              <Flame className="w-4 h-4 text-amber-500" />
              Realtime Database Setup:
            </p>
            <p className="text-slate-600 dark:text-slate-400">
              Make sure Realtime Database is created in project <strong className="font-mono text-indigo-600 dark:text-indigo-400">{projectId}</strong> under Firebase Console.
            </p>
            <div className="pt-1">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs transition-colors"
              >
                <span>Open Firebase Realtime Database Console</span>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="py-2.5 px-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl flex items-center gap-2 transition-all"
          >
            <Flame className="w-4 h-4 text-amber-500" />
